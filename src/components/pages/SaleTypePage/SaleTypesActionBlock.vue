      ...
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn
              color="primary"
              v-bind="props"
              :variant="(selectedSaleTypes?.length ?? 0) > 0 ? 'tonal' : 'text'"
              :disabled="(selectedSaleTypes?.length ?? 0) === 0"
              style="width: 140px; text-align: left"
          >
            {{
              (selectedSaleTypes?.length ?? 0) > 0 ? 'Выбрано ' + (selectedSaleTypes?.length ?? 0) + '  ' : 'Выбрать все'
            }}
            <v-icon v-if="(selectedSaleTypes?.length ?? 0) > 0">mdi-chevron-down</v-icon>
          </v-btn>
        </template>
        <v-list>
          <v-list-item @click="handleDeleteSaleType" >
...
export default defineComponent({
  name: 'SaleTypesActionsBlock',
  props: {
    allSelected: Boolean,
    selectedSaleTypes: Array
  },
  setup(props, {emit}) {
    ...
    const handleDeleteSaleType = () => {
      emit('deleteSaleTypeClicked');
    };
